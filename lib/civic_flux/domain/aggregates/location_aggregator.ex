defmodule CivicFlux.Domain.Aggregates.LocationAggregator do
  ...
  def apply(%__MODULE__{} = state, %LocationAddedToMap{location: location}) do
    %__MODULE__{state | id: location, priority_status: "normal"}
  end

  def apply(%__MODULE__{} = state, %LocationMarkedForAttention{
        location: _location,
        reason: reason
      }) do
    %__MODULE__{state | priority_status: "needs_attention", info: reason}
  end
end
